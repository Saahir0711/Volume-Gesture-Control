...
TH, TI, IX = Hands.HandLandmark.THUMB_TIP, Hands.HandLandmark.THUMB_IP, Hands.HandLandmark.INDEX_FINGER_TIP
...
WIN = "Hand Gesture Control - Use thumb and index to control volume(left hand) and brightness(right hand). Fist for 0 volume/brightness ; Open hand for full volume/brightness"; cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
...
while True:
    ...
    if res.multi_hand_landmarks and res.multi_handedness:
        for i, hand in enumerate(res.multi_hand_landmarks):
            ...
            tips = [Hands.HandLandmark.INDEX_FINGER_TIP, Hands.HandLandmark.MIDDLE_FINGER_TIP,
            Hands.HandLandmark.RING_FINGER_TIP, Hands.HandLandmark.PINKY_TIP]
            fingers = []
            for t in tips:
                if hand.landmark[t].y < hand.landmark[t-2].y:
                    fingers.append(1)
            if (label == "Right" and hand.landmark[TH].x < hand.landmark[TI].x) or (label == "Left" and hand.landmark[TH].x > hand.landmark[TI].x):
                fingers.append(1)
            if len(fingers) == 0:
                dist = 0
            elif len(fingers) == 5:
